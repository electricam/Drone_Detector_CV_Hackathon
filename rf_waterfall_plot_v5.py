...
import threading
import copy
from cot_broadcaster import send_event  # New: Import COT broadcaster

# Hardcoded static location for now (can upgrade later!)
LATITUDE = 38.9072   # Example: Washington DC
LONGITUDE = -77.0369
...
print("\n🎯 Starting tactical RF waterfall scan (with ATAK integration)...")
frame_ready = False
...
try:
    while True:
        ...
        # Spike detection, snapshot, CSV, and CoT sending
        if frame_ready:
            ...
            if len(spikes_idx) > 0:
                ...
                # New: Send CoT event to ATAK
                try:
                    send_event(LATITUDE, LONGITUDE, dominant_freq, downsampled[spikes_idx[0]])
                    print(f"📡 CoT sent: {dominant_freq:.2f} MHz, {downsampled[spikes_idx[0]]:.2f} dB")
                except Exception as e:
                    print(f"❌ Error sending CoT event: {e}")
